...
rt_diff_definition = {
    "stroop": lambda rt_mean_by_group: float(
        rt_mean_by_group["incongruent"] - rt_mean_by_group["congruent"]
    ),
    "ANT": lambda rt_mean_by_group: float(
        (
            (
                rt_mean_by_group["double_incongruent"]
                + rt_mean_by_group["spatial_incongruent"]
            )
            / 2
            - (
                rt_mean_by_group["double_congruent"]
                + rt_mean_by_group["spatial_congruent"]
            )
            / 2
        )
    ),
    "CCTHot": lambda rt_mean_by_group: None,
    "stopSignal": lambda rt_mean_by_group: float(
        rt_mean_by_group["stop_failure"] - rt_mean_by_group["go"]
    ),
    "twoByTwo": lambda rt_mean_by_group: float(
        rt_mean_by_group["900_task_switch"]
        - rt_mean_by_group["900_task_stay_cue_switch"]
    ),
    "WATT3": lambda rt_mean_by_group: None,
    "discountFix": lambda rt_mean_by_group: float(
        rt_mean_by_group["larger_later"] - rt_mean_by_group["smaller_sooner"]
    ),
    "DPX": lambda rt_mean_by_group: float(
        rt_mean_by_group["AY"] - rt_mean_by_group["BY"]
    ),
    "motorSelectiveStop": lambda rt_mean_by_group: float(
        rt_mean_by_group["crit_go"] - rt_mean_by_group["noncrit_signal"]
    ),
}
...
def check_valid_contrasts():
    ...
    possible_task_contrasts = glob.glob(
        "/oak/stanford/groups/russpold/data/uh2/aim1/BIDS/derivatives/output/"
...
def get_design_mat_row_subject(
    ...
    confounds_this_sub = confounds_btwn_sub.loc[
        confounds_btwn_sub["index"] == f"s{subid}"
    ]
    ...
    events_tsv_file = glob.glob(f"{root}/sub-s{subid}/ses-[0-9]/func/*{task}*tsv")[0]
    ...
    if "rt" in model_lev2:
        ...
        rt_mean_by_group = events_for_rt.groupby(rt_trial_grouping[task])[
            "response_time"
        ].mean()
        rt_mean_by_group = rt_mean_by_group.to_frame().transpose()
        rt_mean_by_group.columns = [
            "_".join(col) if type(col) is tuple else col
            for col in rt_mean_by_group.columns.values
        ]
        if task == "twoByTwo":
            rename_dict = {
        ...
        if rt_diff == None:
            raise ValueError((f"Task {task} is not compatable " f"with modelling RT"))
        designs = {
            "rt_diff": np.array([1, float(rt_diff)]),
            "rt_diff_w_confounds": np.array([1, float(rt_diff), age, sex, meanFD]),
            "all_rts_w_confounds": np.array(
                [1] + list(rt_mean_by_group.loc["response_time"]) + [age, sex, meanFD]
            ),
        }
        regressor_names = {
            "rt_diff": ["intercept", "rt_diff"],
            "rt_diff_w_confounds": ["intercept", "rt_diff", "age", "sex", "meanFD"],
            "all_rts_w_confounds": ["intercept"]
            + list(rt_mean_by_group)
            + ["age", "sex", "meanFD"],
        }
...
def get_bold_and_sublist(lev1_task_contrast):
    ...
    lev1_out = (
        f"/oak/stanford/groups/russpold/data/uh2/aim1/BIDS/derivatives/output/"
        f"{task}_lev1_output/task_{task}_rtmodel_{rtmodel}"
    )
    bold_files = sorted(
        glob.glob(
            f"{lev1_out}/contrast_estimates/*_contrast_{lev1_contrast}*rtmodel_{rtmodel}*"
        )
    )
...
def build_desmat_all(
    ...
    if model_lev2 == "rt_diff" or model_lev2 == "rt_diff_w_confounds":
        if lev1_contrast != rt_diff_dv_checker[task]:
            raise ValueError(
                (
                    f"Contrast {lev1_contrast} is not compatable "
    ...
    split_subid_task = pd.DataFrame(
        excluded_subjects.subid_task.str.split("_").to_list(), columns=["subid", "task"]
    )
    ...
    if model_lev2 != "one_sampt":
        ...
        if np.sum(rows_with_missing) > 0:
            summary_missing = pd.DataFrame(
                desmat_all[rows_with_missing, :], columns=regressor_names
            )
            summary_missing["subid"] = np.array(sub_list)[rows_with_missing]
            summary_missing["task"] = task
            summary_missing = summary_missing.reindex(
                columns=["subid"] + ["task"] + regressor_names
            )
            summary_missing.drop("intercept", axis=1, inplace=True)
            summary_missing = pd.merge(
                excluded_subjects, summary_missing, on=["subid", "task"], how="outer"
            )
        else:
...
def make_4d_data_mask(bold_files_final, outdir, lev1_task_contrast):
    ...
    filename_root = f"{outdir}/{task}_lev1_contrast_{lev1_contrast}_rtmodel_{rtmodel}"
...
def make_randomise_files(desmat_final, regressor_names, contrasts, outdir, model_lev2):
    ...
    for contrast in contrasts:
        contrast_def = expression_to_contrast_vector(contrast[0], regressor_names)
    ...
    with open(fts_path, "w") as f:
        f.write(
            f"/NumWaves  {num_contrasts} \n/NumContrasts {num_contrasts}\n \n/Matrix \n"
        )
        np.savetxt(f, np.identity(num_contrasts), fmt="%s", delimiter="\t")


def make_batch_file(outdir, model_lev2, lev1_task_contrast, batch_stub, num_subs):
    task, lev1_contrast, rtmodel = lev1_task_contrast.split(":")
    filename_input_root = (
        f"{outdir}/{task}_lev1_contrast_{lev1_contrast}_rtmodel_{rtmodel}"
    )
    if model_lev2 == "one_sampt":
        randomise_call = (
            f"randomise -i {filename_input_root}.nii.gz"
    ...
    with open(batch_stub) as infile, open(randomise_call_file, "w") as outfile:
        for line in infile:
            line = line.replace("JOBNAME", f"{task}_{lev1_task_contrast}_lev2")
    ...
    randomise_call_file.chmod(
        randomise_call_file.stat().st_mode | stat.S_IXGRP | stat.S_IEXEC
    )
...
def make_html_summary(
    ...
    num_missing = summary_missing.shape[0]
    top_message = (
        f"<h1>Pre analysis summary</h1><br> <h2>Started with N=110, "
    ...
    if desmat_pandas.empty == False:
        ...
        vif_data["VIF"] = [
            variance_inflation_factor(desmat_pandas.values, i)
        ...
        pairgrid.savefig(pairgrid_tmpfile, format="png", dpi=80)
        pairgrid_encoded = base64.b64encode(pairgrid_tmpfile.getvalue()).decode("utf-8")
        html_pairgrid = (
            f"<h2>Regressor summary plots</h2>"
            + "<img src='data:image/png;base64,{}'>".format(pairgrid_encoded)
            + "<br>"
        )

        desmat_fig = plot_design_matrix(desmat_pandas)
        desmat_tmpfile = BytesIO()
        desmat_fig.savefig(desmat_tmpfile, format="png", dpi=60)
        desmat_encoded = base64.b64encode(desmat_tmpfile.getvalue()).decode("utf-8")
        html_desmat = (
            f"<h2>Design matrix</h2>"
            + "<img src='data:image/png;base64,{}'>".format(desmat_encoded)
            + "<br>"
        )
        ...
        plt.figure(
            figsize=(
                0.9 * len(design_column_names),
                0.5 * contrast_matrix.shape[0] + 0.2 * max_len,
            )
        )
        contrast_fig = plt.gca()
        mat = contrast_fig.matshow(
            contrast_matrix, aspect="equal", cmap="gray", vmin=-maxval, vmax=maxval
        )
        ...
        contrast_fig.savefig(contrast_tmpfile, format="png", dpi=75)
        contrast_encoded = base64.b64encode(contrast_tmpfile.getvalue()).decode("utf-8")
        html_contrast = (
            f"<h2>Contrasts</h2>"
            + "<img src='data:image/png;base64,{}'>".format(contrast_encoded)
            + "<br>"
        )
    ...
    cutoff = np.quantile(np.abs(data_nonzero), 0.99)
    mask_img = masking.compute_epi_mask(data4d)
    carpet = plot_carpet(
        data4d,
        mask_img,
        t_r=1,
        title="Raw data for all subjects",
        cmap="Greys",
        vmin=-1 * cutoff,
        vmax=cutoff,
        detrend=False,
    ...
    plt.xlabel("Subjects")
    carpet.set_size_inches(20, 15)
    carpet_tmpfile = BytesIO()
    carpet.savefig(carpet_tmpfile, format="png", dpi=80)
    carpet_encoded = base64.b64encode(carpet_tmpfile.getvalue()).decode("utf-8")
    html_carpet = (
        f"<h2>Brain data summary</h2>"
        + "<img src='data:image/png;base64,{}'>".format(carpet_encoded)
        + "<br>"
    )
    ...
    for idx, ax in enumerate(axs.ravel()):
        if idx < total_n:
            ...
            ax.imshow(
                np.flipud(np.transpose(data4d_array[:, :, 40, idx])),
                cmap="Greys",
                vmin=-1 * cutoff,
                vmax=cutoff,
                aspect="auto",
        ...
        if idx >= total_n:
            ...
            ax.imshow(
                np.zeros(data4d_array[:, :, 40, 1].shape),
                cmap="Greys",
                vmin=-1 * cutoff,
                vmax=cutoff,
            )
    ...
    brain_grid.savefig(brain_grid_tmpfile, format="png", dpi=80)
    brain_grid_encoded = base64.b64encode(brain_grid_tmpfile.getvalue()).decode("utf-8")
    html_brain_grid = (
        f"<h2>Single slice of data for each subject</h2>"
        + "<img src='data:image/png;base64,{}'>".format(brain_grid_encoded)
        + "<br>"
    )
...
def get_parser():
    ...
    parser = ArgumentParser(
        ...
        description=(
            "setup_randomise_lev2: Sets up randomise analysis with "
    ...
    parser.add_argument(
        ...
        action="store",
        help=(
            "Use to specify model. rt_diff and rt_diff_w_confounds only works "
...
if __name__ == "__main__":
    ...
    batch_stub = (
        "/oak/stanford/groups/russpold/data/uh2/aim1/analysis_code/run_stub.batch"
    )
    root = "/oak/stanford/groups/russpold/data/uh2/aim1/BIDS"
    task, lev1_contrast, rtmodel = lev1_task_contrast.split(":")

    outdir = Path(
        f"/oak/stanford/groups/russpold/data/uh2/aim1/BIDS/derivatives/output_CSF-no_motion-yes/"
    ...
    make_batch_file(outdir, model_lev2, lev1_task_contrast, batch_stub, num_subs)
